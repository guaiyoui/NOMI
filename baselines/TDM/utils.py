...
def MAR_mask(X, p, p_obs):
    ...
    # print(X, X.device, idxs_obs, idxs_nas)
    coeffs = pick_coeffs(X, idxs_obs, idxs_nas)
    # print(X, coeffs.device)
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X[:, idxs_obs], coeffs, p)
    # print(X.device, coeffs.device, intercepts.device)
    intercepts = intercepts.detach().cpu()
...
def MNAR_mask_logistic(X, p, p_params =.3, exclude_inputs=True):
    ...
    ber = torch.rand(n, d_na).detach().cpu()
    mask[:, idxs_nas] = ber < ps
    ...
    if exclude_inputs:
        mask[:, idxs_params] = torch.rand(n, d_params).detach().cpu() < p
...
def generate_mask(X_true, missing_prop, mask_type=['MAR', 'MNARL', 'MNARQ', 'MCAR']):
    ...
    if mask_type == 'MAR':
        mask = MAR_mask(X_true, missing_prop, p_obs)
        mask = torch.from_numpy(mask).double()
    elif mask_type == 'MNARL':
        mask = MNAR_mask_logistic(X_true, missing_prop, p_obs)
        mask = torch.from_numpy(mask).double()
